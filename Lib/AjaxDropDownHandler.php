...
if ($ShowFunction == "ShowMaskingId")
    ShowMaskingId($cn, $RequestingValue, $NextCallFunction);
...
function ShowMaskingId($cn, $RequestingValue, $NextCallFunction) {
    $UserName = $RequestingValue;
    $query = "SELECT distinct(MaskingID) FROM MaskingDetail where UserName='$UserName'";
    //echo $query;
    $rs = odbc_exec($cn, $query);
    echo "<select class=\"form-control\" name=\"ShortCodeNumber\" id=\"ShortCodeID\" style=\"width: 200px;\">";

    echo "<option value=\"\">Select Masking ID</option>";

    while ($row = odbc_fetch_array($rs)) {
        echo "<option value=\"" . $row[MaskingID] . "\">" . $row[MaskingID] . "</option>";
    }
    echo "</select>";
}
