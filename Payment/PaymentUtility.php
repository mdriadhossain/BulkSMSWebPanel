...
if (isset($_POST)) {
  ...
  $AccountValSql = "Select CurrentNoOfSMS from MainAccount where UserName='$PurchaseFromAccount'";
    ...
    $MainCurrentSmsVal = $AccountValue[CurrentNoOfSMS];
    $UpdatedMainlNumberOfSms = $MainCurrentSmsVal - $NumberOfSMS;
    ...
    if ($checkSQLarray == FALSE) {
        $insertquery = "INSERT INTO TransactionHistory (UserName, NumberOfSMS,TransactionType, PerSMSRate, Amount,FromMainAccount,TransactionId, LastMainBalance) VALUES ('$UserName', '$NumberOfSMS', '$TransactionType', '$PerSMSRate','$Amount', '$PurchaseFromAccount', '$transactionId', '$UpdatedMainlNumberOfSms')";
        $rs = odbc_exec($cn, $insertquery);
        $insertqueryadmin = "INSERT INTO TransactionHistory (UserName, NumberOfSMS,TransactionType, PerSMSRate, Amount,FromMainAccount, TransactionId,LastMainBalance) VALUES ('admin', ' $NumberOfSMS', 'Debit', 'NULL','NULL','$PurchaseFromAccount', '$transactionId', '$MainCurrentSmsVal')";
       ...
       $MaskingDetailsQuery = "Select count(ID) as counter1 from [dbo].[MaskingDetail] where UserName = '$UserName'";
        $IsUserExistResult = odbc_fetch_array(odbc_exec($cn, $MaskingDetailsQuery));
        $IsUserExist = $IsUserExistResult['counter1'];
        if ($IsUserExist <= 0) {
        ...
        $UpdateMainAccountQuery = "UPDATE MainAccount set CurrentNoOfSMS = $UpdatedMainlNumberOfSms  where username='$PurchaseFromAccount' ";
